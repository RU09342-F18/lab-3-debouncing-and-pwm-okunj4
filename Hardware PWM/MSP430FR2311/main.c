...
int main(void)
{
    ...
    P2SEL0 |= (BIT0);
    P2SEL1 &= ~BIT0;
    P1DIR &= ~BIT1; //set P1.1 button direction to input
    P1REN |= BIT1;  //enable pull up/pull down resistor on P1.1 button
    P1OUT |= BIT1;  //set P1.1 button resistor to pull-up
...
__interrupt void Port_1(void)
{
    ...
    if(P1IES & BIT1)
    {
            if (TB1CCR1<1000)   //if statement for duty cycle under 100%
            {
                TB1CCR1+=100;   //increment duty cycle by 10%
            }
            else if (TB1CCR1 >=1000)  //if statement for duty cycle 100% or more
            {
